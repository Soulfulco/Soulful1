  ...
  router.post("/practitioners", async (req, res) => {
    try {
      ...
      const inPerson = toRate(inPersonRateGbp);
      const online = toRate(onlineRateGbp);
      const groupInPerson = toRate(groupInPersonRateGbp);
      const groupOnline = toRate(groupOnlineRateGbp);
      // At least one rate of any kind (1:1 in-person, 1:1 online, group
      // in-person, group online) must be set — no longer restricted to just
      // the 1:1 rates, since practitioners can now also price group sessions
      // at signup. sessionRateGbp (the base rate the booking system uses) is
      // derived from whichever 1:1 rate is set, falling back to whichever
      // group rate is set if neither 1:1 rate was given.
      const baseRate = inPerson ?? online ?? groupInPerson ?? groupOnline
        ?? (Number.isFinite(Number(sessionRateGbp)) && Number(sessionRateGbp) > 0 ? Number(sessionRateGbp) : null);
      if (baseRate == null) {
        return res.status(400).json({ error: "At least one rate (in-person, online, or group) is required" });
      }
      ...
      const commission = Number.isFinite(proposedCommission) && proposedCommission >= 0 && proposedCommission <= 100
        ? proposedCommission
        : 10;
      const yearsExp = Number(yearsOfExperience);
      const validYearsExp = Number.isFinite(yearsExp) && yearsExp >= 0 && yearsExp <= 80 ? Math.round(yearsExp) : null;
      ...
      const [p] = await db
        ...
        .values({
          ...
          groupInPersonRateGbp: groupInPerson != null ? String(groupInPerson) : null,
          groupOnlineRateGbp: groupOnline != null ? String(groupOnline) : null,
          commissionRatePct: String(commission),
          yearsOfExperience: validYearsExp,
          location,
          qualifications,
          avatarUrl,
          passwordHash,
      ...
      try {
        ...
        if (yearsOfExperience !== undefined) {
          if (yearsOfExperience === null) {
            ...
            const yearsExp = Number(yearsOfExperience);
            if (!Number.isFinite(yearsExp) || yearsExp < 0 || yearsExp > 80) {
              return res.status(400).json({ error: "yearsOfExperience must be between 0 and 80" });
            }
            updates.yearsOfExperience = Math.round(yearsExp);
          }
        }
        if (groupInPersonRateGbp !== undefined) {
          updates.groupInPersonRateGbp = groupInPersonRateGbp != null && Number.isFinite(Number(groupInPersonRateGbp)) && Number(groupInPersonRateGbp) > 0
            ? String(Number(groupInPersonRateGbp))
            : null;
        }
        if (groupOnlineRateGbp !== undefined) {
          updates.groupOnlineRateGbp = groupOnlineRateGbp != null && Number.isFinite(Number(groupOnlineRateGbp)) && Number(groupOnlineRateGbp) > 0
            ? String(Number(groupOnlineRateGbp))
            : null;
        }

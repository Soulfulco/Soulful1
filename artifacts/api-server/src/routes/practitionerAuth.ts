...
router.post("/practitioner/login", async (req, res) => {
  try {
    ...
    if (!p.isActive)
      return res
        .status(403)
        .json({ error: "This account is not active. Please contact Soulful." });
...
router.post("/practitioner/availability", async (req, res) => {
  ...
  try {
    ...
    if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
      return res
        .status(400)
        .json({ error: "Valid startTime and endTime are required" });
    }
    if (end <= start) {
      return res
        .status(400)
...
router.delete("/practitioner/availability/:slotId", async (req, res) => {
  ...
  try {
    ...
    if (!slot || slot.practitionerId !== id)
      return res.status(404).json({ error: "Slot not found" });
    if (slot.isBooked)
      return res
        .status(409)
        .json({ error: "This slot is already booked and cannot be removed" });
    await db
      .delete(timeSlotsTable)
      .where(
        and(
          eq(timeSlotsTable.id, slotId),
          eq(timeSlotsTable.practitionerId, id),
        ),
      );
    res.status(204).send();
  } catch (err) {

...
import { useListSubscriptions, getListSubscriptionsQueryKey, useListPractitionerShowcase, type PractitionerShowcase } from "@workspace/api-client-react";
  ...
  return (
    <Card className="border-2 border-primary/20 rounded-3xl overflow-hidden">
      ...
      <CardContent className="pt-6">
        ...
        <div className={`grid gap-4 ${recommendedPlan ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
          ...
          {recommendedPlan && !("bespoke" in recommendedPlan) && (
            <div className="bg-secondary/10 border border-secondary/30 rounded-2xl p-5">
              ...
              <p className="text-xs text-muted-foreground mt-1">{formatGbp(Number(recommendedPlan.plan.priceGbp))}/mo × 12 = {formatGbp(recommendedPlan.annualCost)}/year</p>
            </div>
          )}
        </div>
...
export default function ForCorporates() {
  ...
  // Both free and paid plans go through the same unified /hr/register
  // endpoint, which creates the company + HR login in one step and, for
  // paid plans, also returns a Stripe checkout URL to redirect to. This
  // replaced a separate createCompany + startCheckout path that hit an
  // admin-only endpoint and would 403 for real (unauthenticated) visitors
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          ...
          if (body.checkoutUrl) {
            // Paid plan — off to Stripe checkout. The session is already
            // active, so they land back logged in regardless of outcome.
            window.location.href = body.checkoutUrl;
            return;
          }
          await refetch();
          toast({ title: "Welcome to Soulful!", description: "Your account is ready." });
          ...
          <div className="grid lg:grid-cols-2 gap-16">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="companyLocation">Head Office Location</Label>
                  <Input
                    id="companyLocation"
                    className="bg-background h-11"
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Used to match your employees with nearby practitioners.</p>
                </div>
                ...
                <Button
                  ...
                  disabled={registering || !selectedPlanId}
                >
                  {registering ? "Creating account..." : selectedPlanId ? (isFreePlan ? "Create free account" : "Complete Registration") : "Select a plan first"}
                </Button>
              </form>
            </CardContent>
            </Card>
